...
<template>
  <view class="informationItem"  wx:for="{{informationItemOption}}" data-set="{{item.talentShowId}}" catchtap="navTo" wx:key="{{index}}">
    <image binderror="imgsErr" data-set="{{index}}" mode="aspectFill" class="in-img {{istab?'isinformation-img':''}}" src="{{item.cover}}"></image>
    <view class="information-title">{{item.title}}</view>
    <view class="information-size">{{language=='ZH'?'精选':'American adult'}}</view>
...
export default class XinWen extends wepy.component {
  props = {};
  ...
  events = {
    putInformationXw(data, status) {
      ...
      for (var i = 0; i < this.informationItemOption.length; i++) {
        ...
        if (this.language == 'ZH') {
          this.informationItemOption[i].createTime = getTime.getZHymd(
            this.informationItemOption[i].createTime
          );
        } else {
          this.informationItemOption[i].createTime = getTime.getENymd(
            this.informationItemOption[i].createTime
          );
        }
      }
  ...
  methods = {
    navTo(e) {
      wx.navigateTo({
        url: 'information/informationDetail?id=' + e.currentTarget.dataset.set
      });
    },
    imgsErr(e) {
