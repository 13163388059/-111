...
<template>
  <view class="report-box">
    <view wx:if="{{isWrite==2}}" class="report-tip">
        ...
        <view catchtap="cancal" class="btn-cancel">{{language=='ZH'?'取消':'Cancel'}}</view>
    ...
    <view catchtap="cancal" wx:if="{{isWrite==1}}" class="isReport"></view>
    <view catchtap="isReport" wx:if="{{isWrite==1}}" class="navToReport">{{language=='ZH'?'举报':'Report'}}</view>
  </view>
</template>
<script>
...
export default class Report extends wepy.component {
  ...
  events = {
    ...
    reportReview(reportId, title) {
      this.isWrite = 1;
      this.targetId = reportId;
      this.title = title;
    }
  };
      ...
      await report(
        {
          ...
          feedbackType: 'report_review',
          relationType: 'review',
          replyCellphone: this.phone,
