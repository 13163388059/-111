...
export default class CourseSearch extends wepy.page {
  ...
  methods = {
    ...
    confirm(e) {
      ...
      if (this.searchValue !== '') {
        this.$parent.globalData.courseSearch = this.searchValue;
        if (wx.getStorageSync('courseHistory') == '') {
          wx.setStorageSync('courseHistory', [this.searchValue]);
        } else {
          ...
          wx.setStorageSync('courseHistory', this.historyList);
        }
        wx.switchTab({
          url: '/pages/course'
        });
  ...
  async init() {
    await hotSearch('course', this.language).then(res => {
  ...
  events = {};
  async onLoad() {
    this.historyList = wx.getStorageSync('courseHistory');
