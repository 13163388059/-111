  ...
  <view class="container">
    <view class="header">
        <view class="search-box {{language=='ZH'?'':'en-search-box'}}">
            ...
            <input placeholder="{{language=='ZH'?'请输入课程名称关键字':'Please enter the keywords'}}" confirm-type="search" class="search-input" bindinput="searchInput" bindconfirm="confirm"/>
        </view>
...
export default class InformationSearch extends wepy.page {
  config = {};
  components = {};

  data = {
    historyList: [],
    newList: ['我是热门新闻', '我是热门新闻', '我是热门新闻', '好新闻'],
    searchValue: '',
    language: 'ZH'
  };
  ...
  methods = {
    ...
    confirm(e) {
      ...
      if (this.searchValue !== '') {
        this.$parent.globalData.informationSearch = this.searchValue;
        if (wx.getStorageSync('informationHistory') == '') {
          wx.setStorageSync('informationHistory', [this.searchValue]);
        } else {
          ...
          wx.setStorageSync('informationHistory', this.historyList);
        }
        wx.switchTab({
  ...
  async init() {
    await hotSearch('zhixun', this.language).then(res => {
      this.newList = res.items;
      this.$apply();
    });
  }
  async onShow() {
    this.historyList = wx.getStorageSync('informationHistory');
